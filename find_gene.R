...
write.csv(dna, file="butler_dna.csv", row.names=F)   ## Comment this out to read in existing input file
# dna <- read.csv("butler_dna.csv", header=F, as.is=T)[[1]]  ## to read in an existing input file
...
# a function to pull out all in same frame (not required): 
...
# Use a loop get all the starts and stops in each frame 1-3:
...
## Assemble ORF dataframe 

for (i in 1:6) {     # i is frame, 1-3 is Forward, 4-6 is Reverse
...
## Print longest ORF to file
cat("\n", "Longest ORF:", "\n", file="butler_orf.txt", sep="", append=T)
i <- orf$orflength == max(orf$orflength)
cat(	"ORF", which(i), 
  		" start: ", orf[i, "start"], 
  		" stop: ", orf[i, "stop"], 
  		" length: ", orf[i, "orflength"], 
