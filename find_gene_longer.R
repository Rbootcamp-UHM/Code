...
input_file <- "rodrigorivero_dna.csv"
dna <- read.csv(input_file, as.is=T)[-1][[1]]   # dataframe to vector
...
for (i in forward) {
  cat(	"ORF", i, 
  		...
  		" frame: ", orf[i, "frame"], 
  		" sequence: ", dna[orf[i, "start"]: (orf[i, "stop"]+2)],  ## use dna
  		"\n", 
  		file=paste(input_file, "output.txt", sep="_"), sep="", append=T)
}
...
for (i in reverse) {
  cat(	"ORF", i, 
  		...
  		" length: ", orf[i, "orflength"], 
  		" frame: ", orf[i, "frame"], 
  		" sequence: ", rcdna[orf[i, "start"]: (orf[i, "stop"]+2)],  ## use rcdna
  		"\n", 
  		file=paste(input_file, "output.txt", sep="_"), sep="", append=T)
}
